-- ============================================================
-- JOINS & AGGREGATE ANALYSIS
-- Selected examples from my database coursework
-- Tool: DB Browser for SQLite
-- ============================================================

-- Artist album counts using LEFT JOIN, GROUP BY and HAVING
SELECT
...
-- Album analysis using JOIN, aggregate functions and HAVING
SELECT
    al.AlbumId,
...
-- Country-level customer and purchase analysis
SELECT
    c.Country,
...
-- Employees without certificates
SELECT COUNT(*) AS 'Employee without a certificate'
FROM Employee e
LEFT JOIN Certificate c ON e.eid = c.eid
WHERE c.eid IS NULL;

-- Qualified pilots linked to aircraft in Flight
SELECT COUNT(DISTINCT e.eid) AS
'Number of pilots qualified to operate aircraft listed in the Flight table'
FROM Employee e
JOIN Certificate c ON e.eid = c.eid
JOIN Flight f ON c.aid = f.aid;

-- Same-invoice track check using multiple joins
SELECT DISTINCT
    c.FirstName || ', ' || c.LastName AS "Full Name",
    i.InvoiceId,
    3 AS "TrackId#1",
...
JOIN invoice_items ii1 ON ii1.InvoiceId = i.InvoiceId AND ii1.TrackId = 3
JOIN invoice_items ii2 ON ii2.InvoiceId = i.InvoiceId AND ii2.TrackId = 9
JOIN invoice_items ii3 ON ii3.InvoiceId = i.InvoiceId AND ii3.TrackId = 15
